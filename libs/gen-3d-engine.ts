...
module miGen3DEngine {
  ...
  export class c3DEngine {
    ...
    createCube(data:iCubeCreate) {
      ...
      var box:iMeshMod = <iMeshMod>BABYLON.MeshBuilder.CreateBox(objname, opt, data.scene);
      ...
      box.onCollide = function () {
        console.warn('oncollide', objname, this)
      };
    ...
    createScene():BABYLON.Scene {
      ...
      scene.clearColor = new BABYLON.Color4(0, 0, 0.5, 1.0);
      ...
      this.ground.onCollide = function () {
        console.warn('oncollide ground')
      };
      ...
      this.table.onCollide = function () {
        console.warn('oncollide table')
      };
      this.table.material = tablemat; //gridshader;
      if (this.opt.hasPhysics)
        this.table.physicsImpostor = new BABYLON.PhysicsImpostor(this.ground, BABYLON.PhysicsImpostor.BoxImpostor, {mass: 0, restitution: 0.9}, this.scene);
      ...
      setTimeout(function () {
        if (state.block_state) {
          state.block_state.forEach(function (frame) {
            ...
            c.isVisible = true;
            if (self.opt.hasPhysics) c.physicsImpostor = new BABYLON.PhysicsImpostor(c,
  ...
  export class cUI3DEngine extends c3DEngine{
    ...
    private createVolumeShadow(mesh:BABYLON.Mesh, scene:BABYLON.Scene, opt:iVolSdwOpt) {
      ...
      if (opt.isCollider) {
        ...
        volumeMesh.onCollide = function () {
          console.warn('vol oncollide', this.name)
        };
      }
      return volumeMesh;
    };
